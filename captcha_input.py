...
def convert_to(data_set, name):
    ...
    for index in range(num_examples):
        image = images[index].reshape([-1]).tolist()  # [height, width, channels] flatten
...
def read_and_decode(filename_queue, one_hot):
    ...
    features = tf.parse_single_example(
        ...
        features={
            ...
            'image': tf.FixedLenFeature([HEIGHT * WIDTH * NUM_CHANNELS], tf.float32),
...
# 读数据主函数
def input_pipeline(one_hot, batch_size, num_epochs=None, name=None):
    if not name:
    ...
    filename_queue = tf.train.string_input_producer(
        ...
        [os.path.join(TFRecord_dir, "{}.tfrecords".format(name))],
        # tf.train.match_filenames_once(name + ".tfrecords"),
        num_epochs=num_epochs,
        shuffle=True
    )
    image, label, height, width, depth = read_and_decode(filename_queue, one_hot=one_hot)
    reshaped_image = tf.reshape(image, tf.pack([height, width, depth]))
    ...
    return _generate_images_and_labels_batch(reshaped_image, label,
...
def main():
    ...
    image_batch, label_batch = input_pipeline(one_hot=False,
                                              batch_size=50, num_epochs=10, name="hehe")
