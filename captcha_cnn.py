...
class CaptchaCNN(object):
	def __init__(self, img_width, img_height, num_of_labels, filter_sizes, num_filters, maxpool_sizes, hidden_size):
		...
		with tf.name_scope("conv-1"):
			...
			conv = tf.nn.conv2d(self.images, filter, strides=[1, 2, 2, 1], padding='VALID', name='conv')
		...
		with tf.name_scope('conv-2'):
			...
			h = tf.nn.relu(tf.nn.bias_add(conv, b), name="relu")
			pooled2 = tf.nn.max_pool(h, ksize=[1, maxpool_sizes[1], maxpool_sizes[1], 1], strides=[1, 1, 1, 1],
									 padding='VALID', name='maxpool')
		...
		with tf.name_scope('conv-3'):
			...
			h = tf.nn.relu(tf.nn.bias_add(conv, b), name="relu")
			pooled3 = tf.nn.max_pool(h, ksize=[1, maxpool_sizes[2], maxpool_sizes[2], 1], strides=[1, 2, 2, 1],
									 padding='VALID', name='maxpool')
		...
		with tf.name_scope('loss'):
			...
			self.correct_indices = tf.range(0, 6 * batch_size) * num_of_labels + flatten_labels
			flatten_scores = tf.reshape(self.scores, [-1, ])
			correct_scores = tf.gather(flatten_scores, self.correct_indices)
