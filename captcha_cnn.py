...
class CaptchaCNN(object):
    def __init__(self, filter_sizes, num_of_filters, filter_strides, maxpool_sizes, maxpool_strides, input_channels,
                 hidden_sizes, num_of_labels, num_of_classes):
    ...
    def conv_maxpool(self, inputs, filter_size, num_of_filter, filter_stride, maxpool_size, maxpool_stride):
    ...
    def full_connect(self, inputs, weights_shape, biases_shape):
        with tf.device('/cpu:0'):
    ...
    def full_connect_activate(self, inputs, weights_shape, biases_shape, activation='relu'):
        if activation == 'relu':
            return tf.nn.relu(self.full_connect(inputs, weights_shape, biases_shape))
        elif activation == 'sigmoid':
            return tf.nn.sigmoid(self.full_connect(inputs, weights_shape, biases_shape))
        elif activation == 'tanh':
            return tf.nn.tanh(self.full_connect(inputs, weights_shape, biases_shape))
        else:
    ...
    def flow(self, inputs):
        ...
        for i in range(len(self.filter_sizes)):
            if i == 0:
                ...
                inputs_ = self.conv_maxpools[-1]
            with tf.variable_scope('conv-pool-{}'.format(i + 1)):
                conv_pool = self.conv_maxpool(inputs_,
                                              filter_size=self.filter_sizes[i],
                                              num_of_filter=self.num_of_filters[i],
                                              filter_stride=self.filter_strides[i],
                                              maxpool_size=self.maxpool_sizes[i],
                                              maxpool_stride=self.maxpool_strides[i])
            self.conv_maxpools.append(conv_pool)
        ...
        for i in range(len(self.hidden_sizes)):
            with tf.variable_scope('full-connect-{}'.format(i + 1)):
                if i == 0:
                    ...
                    inputs_ = tf.reshape(self.conv_maxpools[-1], [batch_size, -1])
        ...
        with tf.variable_scope('outputs') as scope:
            for i in range(self.num_of_labels):
                with tf.variable_scope('part-{}'.format(i)):
                    fc_part = self.full_connect(self.full_connects[-1],
